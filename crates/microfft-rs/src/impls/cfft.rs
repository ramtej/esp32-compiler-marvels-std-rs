...
pub(crate) trait CFft {
    ...
    fn transform(x: &mut [Complex32]) -> &mut [Complex32] {
        ...
        for (i, v) in x.iter().enumerate() {
            println!("arch[{}]:bit_reverse_reorder[{}] = {}", ARCH, i, v);
        }
        Self::compute_butterflies(x);
        for (i, v) in x.iter().enumerate() {
            println!("arch[{}]:compute_butterflies[{}] = {}", ARCH, i, v);
        }
        x
    }
    ...
    fn compute_butterflies(x: &mut [Complex32]) {
        ...
        for (i, v) in x.iter().enumerate() {
            println!("arch[{}]:compute_butterflies.input_x[{}] = {}", ARCH, i, v);
        }
        ...
        println!("arch[{}]:table_len = {}", ARCH, table_len);
        ...
        for k in 1..u {
            ...
            println!("arch[{}]:twiddle = {}", ARCH, twiddle);
        ...
        for (i, v) in x.iter().enumerate() {
            println!("arch[{}]:compute_butterflies.x_1[{}] = {}", ARCH, i, v);
        }
        ...
        for (i, v) in x.iter().enumerate() {
            println!("arch[{}]:compute_butterflies.x_2[{}] = {}", ARCH, i, v);
        }
    }
}
